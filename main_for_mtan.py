...
if __name__ == '__main__':
    ...
    if command_variable.mode == 'train':
        ...
        try:
            ...
            # print(**config['dataset']['train']['GetDataset'])
        ...
        # create model
        dim = 30
        rec = mtan_model.enc_mtan_rnn(
            dim, torch.linspace(0, 1., 128), 32, 32, 
            embed_time=128,  num_heads=1).to(device)
        dec = mtan_model.dec_mtan_rnn(
            dim, torch.linspace(0, 1., 128), 32, 32, 
            embed_time=128, num_heads=1).to(device)
        classifier = mtan_model.create_classifier(32, 32).to(device)
        params = (list(rec.parameters()) + list(dec.parameters()) + list(classifier.parameters()))
        

        criterion = nn.CrossEntropyLoss()

        if command_variable.fname is not None:
            checkpoint = torch.load(command_variable.fname)
